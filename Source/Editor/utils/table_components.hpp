...
inline auto reset_button = [](char const* id, auto& val, ImVec4 const color, f32 const reset_val) {
...
void draw_vec3(
  ...
  auto vec3_component = [setter, reset_vec](char const* name, auto& val, DragProperties const prop) {
    ...
    changes |= reset_button("###X", val.x, ImVec4 {0.8F, 0.1F, 0.15F, 1.0F}, prop.reset_value);
    ImGui::SameLine();
    ...
    changes |= reset_button("Y", val.y, ImVec4 {0.2F, 0.7F, 0.2F, 1.0F}, prop.reset_value);
    ImGui::SameLine();
    ImGui::PushItemWidth(avail_width);
    ...
    changes |= reset_button("Z", val.z, ImVec4 {0.1F, 0.25F, 0.8F, 1.0F}, prop.reset_value);
    ImGui::SameLine();
    ImGui::PushItemWidth(avail_width);
