...
class Editor {
public:
    Editor(window::Info& window_info);
    ~Editor();
    void init();
    void run();
    void terminate();
...
template<graphics::HRI Gfx, window::Manager<Gfx> Window>
Editor<Gfx, Window>::Editor(window::Info& window_info) : explorer_("C:"), viewport_(window_info) {
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
inline void Editor<graphics::Dx12, window::Win32>::init() {
    ...
    ImGui_ImplDX12_Init(graphics.device(), 3, DXGI_FORMAT_R8G8B8A8_UNORM, graphics.heaps().srv.get(),
