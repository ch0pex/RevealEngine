...
void SceneGraph::drawSceneGraph() {
    ...
    const int n_leaf_start = std::max(static_cast<i32>((window_pos.y - last_item.y) / item_size.y), 0);
    const int n_leaf_can_draw = std::min(static_cast<i32>(window_size.y / item_size.y),
                                         static_cast<i32>(core::scene.count()) - n_leaf_start);
