...
#include "engine/core/misc/timer.hpp"
...
inline void getPoly(std::string const& line, std::vector<detail::FaceElem>& primitives) {
...
inline void getTriangle(std::string const& line, std::vector<detail::FaceElem>& primitives) {
  std::array<std::array<u32, 3>, 3> elem;
  std::sscanf(
...
inline std::optional<render::Mesh> import_obj(std::string_view const path) {
  render::Mesh mesh;
